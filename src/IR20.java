   ...
   for (int i = 0; i < numberOfTerms; i++){
     ...
     for (int j = lo; j < hi; j++){
       ...
       for (int k = 0; k < numberOfTopics; k++){
    	   ...
    	   {
    		   topicStrength = 
    			          ((topicTermConfusion[k][i] + gamma)-tfs[j])
    			             / ((topicSizes[k] + numberOfTerms * gamma)-tfs[j])
    			          * ((docTopicConfusion[postings[j]][k] + alpha)-tfs[j])
    			          / ((docLengths[postings[j]] + numberOfTopics * alpha)-tfs[j]);
    	   }
    	   else
    	   {
    		   topicStrength = 
