// IR7.java CS6054 2015 Cheng
// BIM ranking with pseudo relevance feedback
// Usage:  java IR7 isrInvertedTf.txt isr4.txt
// Usage:  java IR7 adInvertedTf.txt ad14.txt
    ...
    for (int termID = 0; termID < numberOfTerms; termID++){
      ...
      for (int j = lo; j < hi; j++) 
         if (topDocSet.contains(postings[j])) vt++;
      		pt[termID] = (Math.abs(vt)+0.5)/(topDocSet.size()+1);
      		System.out.println(topDocSet.size()+"asdadsad");
      		ut[termID] = ((dfs[termID]-Math.abs(vt)+0.5)/(numberOfDocs-topDocSet.size()+1));
    }
  }
 ...
 void BIMScores(String query){
   ...
   System.out.println(terms[j] + " " + 
      ...
      decimalf.format(ut[termID]));
   		double a11=((pt[termID])/(1-pt[termID]));
   		double ct = Math.log(a11)+Math.log((1-ut[termID])/ut[termID]);
