...
// binary search
   ...
   for (int j = 0; j < len; j++){
     ...
     if (termID >= 0){
       ...
       for (int k = postingsLists[termID];
        ...
        // BM25, (11.32)
        scores[1][docID] += ((idfs[termID]*((double)k1+1)*tfs[k])/((double)k1*((1-b)+b*(ld/lave))));
        // Jelinek-Mercer, (12.10) 
        scores[2][docID] *= ((lambda*(tfs[k]/T))+jmC) / jmC;
        // Dirichlet, (12.11)
 ...
 public static void main(String[] args){
   IR8 ir8 = new IR8();
   ir8.readInvertedIndex(args[0]);
   ir8.readTitles(args[1]);
   ir8.precompute();
   ir8.answerQueries();
 }
}