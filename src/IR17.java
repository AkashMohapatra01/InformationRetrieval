...
// you need to change this for single-link in the second run
 double sim(int i, int k1, int k2){ // Figure 17.8 complete-link
   ...
   return C[i][k1] >= C[i][k2] ? C[i][k1] : C[i][k2]; // Maximum Distance
 }       
 ...
 void subtree(int index){
   ...
   if (k < 0){
    // Your code to place an indentation of depths[index] spaces before title
	int j = depths[index]; for (; j >= 0; j--)
			System.out.print("\t");
 ...
 public static void main(String[] args){
  IR17 ir17 = new IR17();
  ir17.readInvertedIndex(args[0]);
  ir17.readTitles(args[1]);
